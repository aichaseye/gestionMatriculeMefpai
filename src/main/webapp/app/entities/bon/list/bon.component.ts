...
export class BonComponent implements OnInit {
  ...
  constructor(protected bonService: BonService, protected modalService: NgbModal, protected parseLinks: ParseLinks) {
    this.bons = [];
  ...
  loadAll(): void {
    ...
    this.bonService
      ...
      .subscribe({
        next: (res: HttpResponse<IBon[]>) => {
          this.isLoading = false;
          this.paginateBons(res.body, res.headers);
        },
        error: () => {
          this.isLoading = false;
        },
  ...
  protected paginateBons(data: IBon[] | null, headers: HttpHeaders): void {
